...
When the program is installed correctly, invoking the appropriate **RUN_ME** specified in [Starting Comet.Photos](#starting-cometphotos) will open up a browser window (or a new tab in an existing browser window) to http://localhost:8082, and load a 3D model of 67P. The **Matches** field in the control panel  should report "27242/27242 matches", indicating that initially every is loaded, and none are filtered out. On startup, the program checks key parts of its installation, and prints out messages to convey its state. If an error occurs, this output often points to a missed step in the installation, or a configuration inconsistency. Please report any problems as described in section on [How to Report Issues](#how-to-report-issues), and include this output. People wanting to further test the installation can follow along with the [Step-By-Step Example](#step-by-step-example) on their own computer, although expect small differences based on variations in the precise region of interest that is painted.
