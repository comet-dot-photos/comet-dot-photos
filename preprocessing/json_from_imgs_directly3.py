...
# one compiled regex that finds ANY object block and names the pieces
_OBJECT_RE = re.compile(
    r'(?ims)'                                     # i:casefold m:^/$ per line s:dotall
    r'^[ \t]*OBJECT[ \t]*=[ \t]*"?([A-Z0-9_]+)"?[ \t]*\r?$\s*'  # \1 = object name token
    r'(.*?)'                                       # \2 = body (non-greedy)
    r'^[ \t]*END_OBJECT[ \t]*=[ \t]*"?\1"?[ \t]*\r?$\s*'        # must close same name
)

def getObjectString(header: str, want: str = "IMAGE") -> str:
    want = want.upper()
    for m in _OBJECT_RE.finditer(header):
        name = m.group(1).upper()
        if name == want:            # only take the IMAGE block, never PB_IMAGE
            return m.group(2)
    return ""   # not found
...
def extractViewData(file):
    ...
    if xres != yres:
        ...
        return None
    if (xres != SCREENRES):
        print(f"Non-default resolution: {xres} x {yres} in {file}")
    view['rz'] = xres
...
def jpgFileExists(file, jpgDir):
    ...
    if not isThere: print(f"CHECK THIS - MISSING JPG: {jpg_file}")
    return isThere
